-- ============================================
-- StrongHub - V1__baseline.sql  (PostgreSQL 13+)
-- Multi-tenant + Auditoría + Catálogos + B2B
-- SOLO DDL (sin DML ni DROPs)
-- ============================================
...
-- Índices de soporte (lecturas frecuentes por rol)
...
-- Índices de soporte (lecturas por rol)
CREATE INDEX IF NOT EXISTS idx_gym_user_roles_role_id ON gym_user_roles(role_id);
...
CREATE TABLE members (
  ...
  email VARCHAR(120) NOT NULL,                -- (sin UNIQUE a nivel columna)
  phone VARCHAR(40),
  date_of_birth DATE,
  gender VARCHAR(20),
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
...
-- Unicidad lógica con soft-delete
...
CREATE TABLE trainers (
  ...
  phone VARCHAR(40),
  email VARCHAR(120),                          -- (sin UNIQUE a nivel tabla)
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  created_by INT NULL,
  updated_by INT NULL,
...
-- Reemplaza índice previo no-único por índice único parcial (soft-delete)
DROP INDEX IF EXISTS idx_trainers_email_active;
CREATE UNIQUE INDEX IF NOT EXISTS uq_trainers_email_active
...
CREATE TABLE plans (
  ...
  UNIQUE (gym_id, name),
  UNIQUE (gym_id, id), -- necesario para FK compuesta desde subscriptions
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  created_by INT NULL,
  updated_by INT NULL,
...
CREATE TABLE class_sessions (
  ...
  UNIQUE (gym_id, id), -- para FK compuesta desde reservas/asistencia
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  created_by INT NULL,
  updated_by INT NULL,
...
-- =================================
-- 10) Coherencia trainer ↔ gym (FKs compuestas)
-- (colocadas al final para asegurar existencia de tablas destino)
-- =================================
ALTER TABLE class_sessions
  ADD CONSTRAINT fk_class_session_gym_trainer
  FOREIGN KEY (gym_id, trainer_id)
  REFERENCES gym_trainers(gym_id, trainer_id);

ALTER TABLE subscriptions
  ADD CONSTRAINT fk_subscription_gym_trainer
  FOREIGN KEY (gym_id, trainer_id)
  REFERENCES gym_trainers(gym_id, trainer_id);

-- =========================
-- FIN V1__baseline.sql
-- =========================
